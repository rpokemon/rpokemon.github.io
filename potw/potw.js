...
potw_header = `Hello all! It's time for another Pokemon of the Week thread! **Don't forget to vote for next week's Pokemon of the Week at the bottom of this post!**
...
potw_pokemon_template_body = `**#{0} {1}** (Japanese {2} {3})
...
potw_pokemon_template_imgur = `[Artwork by /u/{0} for /r/Pokemon Draws Pokemon]({1})

`

potw_pokemon_template_links = `{1} on - [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/{1}) | [Serebii](http://www.serebii.net/pokedex-sm/{0}.shtml) | [Smogon](http://www.smogon.com/dex/sm/pokemon/{1}/) | [Pokemon.com](http://www.pokemon.com/us/pokedex/{1})
...
potw_poll = `**To determine next week's PotW, we use [this site](http://randompokemon.com/) to generate three random, fully-evolved Pokemon. Then, we put it to a vote. Which of the following three Pokemon should be next week's PotW?**
...
pokemon_list = null;
$.getJSON("dex.json", function (result) { pokemon_list = result });
evolved_pokemon = []
...
function get_potw() {
    ...
    pokemon_of_the_week = document.getElementById("potw-selection-text").value;

    for (var pokemon in pokemon_list) {
        obj = pokemon_list[pokemon]
        if (pokemon_of_the_week.toLowerCase() == obj["name"]["english"].toLowerCase()) {
            ...
            if (obj["reddit_draws_url"]) {
                potw_output += potw_pokemon_template_imgur.format(obj["reddit_draws_url"][1], obj["reddit_draws_url"][0])
            }
            ...
            if (obj["previous_evolutions"]) {
                ...
                for (var i = 0; i < obj["previous_evolutions"].length; i++) {
                    ...
                    if (evo_obj["reddit_draws_url"]) {
                        potw_output += potw_pokemon_template_imgur.format(evo_obj["reddit_draws_url"][1], evo_obj["reddit_draws_url"][0])
                    }
            ...
            potw_output += potw_poll.format(pokemon_list[evolved_pokemon[0]]["name"]["english"], pokemon_list[evolved_pokemon[1]]["name"]["english"], pokemon_list[evolved_pokemon[2]]["name"]["english"])
